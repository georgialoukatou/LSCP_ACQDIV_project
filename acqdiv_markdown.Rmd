...
#For Chintang, remove Nepali morphemes and words:
morpheme_data<-data.frame(morphemes)
clean_corpus<-morpheme_data[ , c('morpheme_language', "morpheme")]
language_data<-clean_corpus[clean_corpus$morpheme_language=="Nepali", ]
list.var<-unique(language_data$morpheme)
...
sed 's/-//g' 
...
sed '/^\s*$/d' cleaned_file.txt > extracted_corpus1.txt
...
sed -e 's/&PRES//g' -e 's/&IMP//g' -e 's/&ADV//g' -e 's/&SGER//g' -e 's/&NEG//g' -e 's/&POL//g' -e 's/&CONN//g' -e 's/&PAST//g' -e 's/&COND//g' -e 's/&PASS//g' -e 's/&QUOT//g' -e 's/&CAUS//g' -e 's/&P$  -e 's/\^//g' -e "s/\'//g"  -e 's/)//g' -e 's/(//g' -e 's/&//g'  ${VERSION}/divided_corpus.txt > ${VERSION}/divided_corpus_na.txt
