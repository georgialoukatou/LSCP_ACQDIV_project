...
[1]
This script has been created to show the pipeline for segmenting the acqdiv corpus. The languages used are Chintang, Japanese and Russian.

First of all, choose the language with which you want to start.
If you wish to evaluate them on their morpheme level (not provided for Russian), declare it below at the LEVEL variable.

[2]
Add the path to the database, the script folder and to the place you want to save the extracted corpus.

[3]
Extract the corpus using the file sel_clean.r and save it in a file named extracted_corpus.txt. This script takes the CDS from the ACQDIV database and saves it, one utterance per line. Then, it removes all NAs.

[4]
If and only if processing the Russian corpus, follow the instructions below and then move immediately to [9]. The corpus will be phonologized by festival, so a different pipeline is needed.
If you process Chintang or Japanese, skip this step.
...
[6] 
Then, we can clean and phonologize the sub-corpora. For this, need to create a for loop.


[7]
If processing the Japanese morpheme corpus, please add a space where - before phonologizing. If not the morpheme annotations will be lost during the cleaning phase.
...
[9]
This command would give the descriptive statistics of the corpus. Attention! The commands in steps 9,10,11 and 12 can only be ran in oberon, because they need the new wordseg package.

[10]! In oberon
Now it's segmentation time. Always in the for loop, run TPs and DiBS with the new wordseg package on oberon. We should also declare your gold and tags files. 

[11]! In oberon
I run AG with the old wordseg version for now. The script segment_jap.py works for both Japanese and Chintang, so there is no need to change it. ${PATH_TO_OLD_WORDSEG} is needed. Attention! AGu takes a lot of time. Then, we can close the loop.

[12]! In oberon
There is a simple baseline available which cuts every syllable/phoneme with probability 0.5.
